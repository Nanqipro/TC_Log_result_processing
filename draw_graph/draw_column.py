...
#
# # 读取数据
# file_path = './all_log/time_all.xlsx'  # 替换为实际文件路径
...
# bar_positions = np.arange(len(datasets)) * (10 * bar_width + spacing)  # 使每组数据集之间有间隔
#
# # 绘制柱状图并设置不同的颜色
# ax1.bar(bar_positions - 4.5 * bar_width, Bisson, bar_width, label='Bisson', color='#BDB76B', edgecolor='black', linewidth=1)
# ax1.bar(bar_positions - 3.5 * bar_width, Fox, bar_width, label='Fox', color='#6495ED', edgecolor='black', linewidth=1)
# ax1.bar(bar_positions - 2.5 * bar_width, Green, bar_width, label='Green', color='#E6E6FA', edgecolor='black', linewidth=1)
# ax1.bar(bar_positions - 1.5 * bar_width, H_INDEX, bar_width, label='H_INDEX', color='#FFE4C4', edgecolor='black', linewidth=1)
# ax1.bar(bar_positions - 0.5 * bar_width, Hu, bar_width, label='Hu', color='#FFF5EE', edgecolor='black', linewidth=1)
# ax1.bar(bar_positions + 0.5 * bar_width, TriCore, bar_width, label='TriCore', color='#D8BFD8', edgecolor='black', linewidth=1)
# ax1.bar(bar_positions + 1.5 * bar_width, Polak, bar_width, label='Polak', color='#87CEEB', edgecolor='black', linewidth=1)
# ax1.bar(bar_positions + 2.5 * bar_width, GroupTC_BS, bar_width, label='GroupTC_BS', color='#FF8C00', edgecolor='black', linewidth=1)
# ax1.bar(bar_positions + 3.5 * bar_width, TRUST, bar_width, label='TRUST', color='#B0E0E6', edgecolor='black', linewidth=1)
# ax1.bar(bar_positions + 4.5 * bar_width, GroupTC_HS, bar_width, label='GroupTC_HS', color='#DAA520', edgecolor='black', linewidth=1)
#
# # 标记缺失值
# def mark_missing_values(ax, data, positions, width, label):
#     for i, value in enumerate(data):
#         if pd.isna(value):  # 如果值是缺失值
#             ax.text(positions[i], 0, 'Missing', ha='center', va='bottom', color='red', fontsize=12, fontweight='bold')
#
#
#
...
# ax1.set_xticklabels(datasets, fontsize=18, rotation=20, ha='center')
#
# # 添加图例
# fig.legend(loc='upper left', bbox_to_anchor=(0.07, 0.95), fontsize=18)
#
# # 设置边框线宽
# ax = plt.gca()
...
max_value = max(df[['Bisson', 'Fox', 'Green', 'H-INDEX', 'Hu', 'Polak', 'TriCore', 'TRUST', 'GroupTC-BS', 'GroupTC-HS']].max())
