...
# 读取数据
file_path = '../../excel/real_world_graph_profile.xlsx'  # 替换为实际文件路径
df = pd.read_excel(file_path, sheet_name='global_load_requests')
...
def plot_bar_with_missing(ax, positions, data, label, color, missing_color, bar_width, max_value):
    ...
    for i, value in enumerate(data):
        if pd.isna(value):  # 如果数据缺失
            ...
            # 在柱子顶部添加红色叉号标记
            ax.text(positions[i], max_value, '×', ha='center', va='bottom', fontsize=16, color='red')  # 红色叉号标记
        else:
            ax.bar(positions[i], value, bar_width, color=color, edgecolor='black', linewidth=1)  # 绘制正常柱

# 绘制柱状图，去掉 `label` 参数来避免重复
plot_bar_with_missing(ax1, bar_positions - 4.5 * bar_width, Polak, 'Polak', '#f6c89a', missing_color, bar_width, max_value)
plot_bar_with_missing(ax1, bar_positions - 3.5 * bar_width, Green, 'Green', '#a5d4a1', missing_color, bar_width, max_value)
plot_bar_with_missing(ax1, bar_positions - 2.5 * bar_width, Bisson, 'Bisson', '#c1b3d5', missing_color, bar_width, max_value)
...
ax1.set_xlim([-0.6, len(datasets) * (10 * bar_width + spacing) -0.64])
# 设置 y 轴的刻度标签大小
...
# 保存图形为 PDF 文件
plt.savefig('../../pdf/global_load_requests_v6.pdf', format='pdf')
