...
bar_positions = np.arange(len(datasets)) * (4 * bar_width + spacing)  # 使每组数据集之 间有间隔
...
ax1.bar(bar_positions[2], 0, bar_width, label='GroupTC-BS', color='#91d0fc', edgecolor='black', linewidth=1)  # 假的柱子只为图例显示
ax1.bar(bar_positions[3], 0, bar_width, label='GroupTC-HS', color='#f2e5c1', edgecolor='black', linewidth=1)  # 假的柱子只为图例显示
...
# 保存图形为 PDF 文件
plt.savefig(r'D:\BaiduNetdiskDownload\G500_change_E.pdf', format='pdf')
